...
async function main() {
  ...
  try {
    ...
    // 4) 무브 릴레이 (iOS 클라이언트와 동일한 인코딩: from/to/promotion)
    const move = { from: 51, to: 35 };
    host.send(JSON.stringify({ type: 'move', move }));
    const relayed = await guest.next();
    assert(relayed.type === 'move' && relayed.move.from === 51 && relayed.move.to === 35,
      '무브 릴레이 (호스트→게스트)');

    const move2 = { from: 9, to: 1, promotion: 'queen' };
    guest.send(JSON.stringify({ type: 'move', move: move2 }));
    const relayed2 = await host.next();
    assert(relayed2.type === 'move' && relayed2.move.to === 1 && relayed2.move.promotion === 'queen',
      '무브 릴레이 (게스트→호스트, 프로모션 포함)');
