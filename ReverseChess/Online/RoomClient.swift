...
/// 엔진이 결정적이므로 서버는 무브를 해석하지 않는 단순 릴레이면 충분하다.
@MainActor
final class RoomClient: ObservableObject {
    enum ConnectionState: Equatable {
        case idle
        case connecting
        /// 방 생성 완료 — 상대 입장 대기 중
        case waitingForOpponent(code: String)
        /// 매칭 완료 — 내가 둘 색 (선공 = 흑, 서버가 랜덤 배정)
        case matched(me: PieceColor)
        case opponentLeft
        case failed(String)
    }
    ...
    private func handle(_ msg: ServerMessage) {
        ...
        case "joined":
            myIndex = msg.player
        case "start":
            // first = 선 플레이어의 인덱스. 리버스 체스는 흑이 선공이므로 선 = 흑.
            guard let myIndex, let first = msg.first else { return }
            let me: PieceColor = (myIndex == first) ? .black : .white
            state = .matched(me: me)
        case "move":
